...
<div class="entry" data-type="poll" data-co="{{ $uniquid }}" @if(isset($entry->id)) data-entry-id="{{ $entry->id }}"
    ...
    <div class="moredetail text">
        ...
        <div class="detailhide" style="display:none">
            ...
            <div class="inpunting">
                ...
                'message','id' => 'edit', 'placeholder' => trans('addpost.entry_body')]) !!}
            </div>
        </div>
    ...
    <div class="moredetail questionmore">
        ...
        <div class="answertypeselection">
            <a class="button @if(isset($entry->video)) {{ $entry->video=='3' ?  'button-orange' : 'button-white'}} @else button-orange @endif clickanswertype"
                data-style="thlist" data-query="3" @if(isset($entry->video))
                {{ $entry->video=='3' ? 'data-curtype=on' : '' }}@else data-curtype="on" @endif><i
                    class="fa fa-th-list "></i></a>
            <a class="button @if(isset($entry->video)) {{ $entry->video=='1' ?  'button-orange' : 'button-white'}} @else button-white @endif clickanswertype"
                data-style="thdefault" data-query="1" @if(isset($entry->video))
                {{  $entry->video=='1' ? 'data-curtype=on' : '' }} @endif><i class="fa fa-th"></i></a>
        ...
        <div id="answer{{ $uniquid }}"
            class="answers clearfix @if(isset($entry->video)) @if($entry->video == '1')thdefault @else {{  $entry->video == "2" ?  'thlarge' : 'thlist' }}@endif @else thlist @endif"
            style="display:flex;flex-wrap: wrap;:0;border:0;padding:0;margin:0;margin-left:-15px;background: transparent;">
            ...
            @foreach($post->entries()->where('type', 'answer')->where('source',
            (int)$entry->id)->orderBy('order')->get() as $answeras)
            @if(isset($answeras))
        ...
        <div class="clearfix"></div>
        <a class="button button-white button-full submit-button entry_fetch answerbutton clearfix" data-method="Get"
            data-target="answer{{ $uniquid }}" data-puttype="append" data-type="pollanswerform"
            href="{{ action('FormController@addnewform') }}?addnew=pollanswer"><i
