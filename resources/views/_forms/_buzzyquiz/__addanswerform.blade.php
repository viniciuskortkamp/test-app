...
<div  class="answer-wrapper">
    ...
    @if(Request::query('qtype')=='trivia' || isset($answer->answer_type) && $answer->answer_type == 'trivia')
        <div class="inpunting " style=" padding:10px 5px;font-size:12px:font-weight:bold">
            ...
            <input class="cd-input" name="assign" type="radio" {{  isset($answer->video) ? $answer->video == 'on' ? 'checked="checked"' : '' : '' }} value="on" data-type="assign" id="correct-{{$idoa}}" style="display: inline;margin-right: 6px;margin-left: 5px;margin-top: 4px;width: auto;cursor:pointer"> <label for="correct-{{$idoa}}" style="display:inline-block;cursor:pointer">{!!  trans('buzzyquiz.correctanswer')  !!}</label>
        </div>
    @else
