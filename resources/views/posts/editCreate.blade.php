                            ...
                            <select id="tagcats" class="demo-default" name="category" multiple
                                ...
                                <optgroup label="">
                                    <option value="{{ $categorys->id . ',' }}"
                                        {{ strpos($post->categories, '"' . $categorys->id . ',"') ? 'selected' : '' }}>
                                        {{ $categorys->name }}</option>
                                    @foreach (\App\Categories::byType($categorys->id)
                                    ->orderBy('name')
                                    ->get()
                                    as $i => $cat)
                                    <option value="{{ $categorys->id . ',' . $cat->id . ',' }}"
                                        {{ strpos($post->categories, '"' . $categorys->id . ',' . $cat->id . ',"') ? 'selected' : '' }}>
                                        <b>{{ $categorys->name }}</b> / {{ $cat->name }}</option>
                                    @foreach (\App\Categories::byType($cat->id)
                                    ->orderBy('name')
                                    ->get()
                                    as $io => $catq)
                                    <option value="{{ $categorys->id . ',' . $cat->id . ',' . $catq->id . ',' }}"
                                        {{ strpos($post->categories, '"' . $categorys->id . ',' . $cat->id . ',' . $catq->id . ',"') ? 'selected' : '' }}>
